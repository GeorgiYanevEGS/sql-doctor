...
python -m pytest tests/ -v         # runs all 74 tests
...
What's implemented: parser, 11 skills (with selectivity-, loop-, spill-,
child-shape-, low-estimate-, heap-fetch-, outer-child-estimate-, and
parallel-worker-awareness), provider abstraction (3 backends), schema
introspection, validator, coverage ledger, CLI wiring, 74 tests:

- **35 skill-matching tests** — synthetic EXPLAIN JSON, no DB required.
  Of these, 6 are regression tests written after real false positives
  were found and fixed during live testing.
- **20 negative tests** — each proves a specific (skill, node type) pair
...
the `test` job runs all 74 tests, and the `ledger-integrity` job
